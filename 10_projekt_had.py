"""
PROJEKT: Textová hra – Had v bludišti
=======================================
Závěrečný projekt! Hra v terminálu.
Had se pohybuje, sbírá jídlo a roste.

Ovládání: w (nahoru), s (dolů), a (vlevo), d (vpravo), q (konec)
"""
...
def vykresli(had, jidlo, skore):
    ...
    print(f"  Skóre: {skore}   |   Ovládání: W A S D   |   Konec: Q")
    print("+" + "-" * SIRKA + "+")
...
def hraj():
    ...
    smer = [1, 0]   # začínáme jdeme doprava
    skore = 0
    ...
    while True:
        ...
        if not (0 <= nova_hlava[0] < SIRKA and 0 <= nova_hlava[1] < VYSKA):
            vykresli(had, jidlo, skore)
            print(f"\nBUM! Narazil jsi do zdi. Konečné skóre: {skore}")
            break

        # kousl sám sebe?
        if nova_hlava in had:
            vykresli(had, jidlo, skore)
            print(f"\nBUM! Kousl jsi se. Konečné skóre: {skore}")
            break
